...
API_RECIPE_INCLUDE_FIELDS = settings.API_RECIPE_INCLUDE_FIELDS


def get_or_set_recipe_best_today_cached() -> Dict[str, str]:
    cache_name = f'{__name__}_best_today'

    rendered = cache.get(cache_name)
    if not rendered:
        recipes = get_best_recipes_by_start_date(datetime.now(), count=1)
        rendered = render_recipe(recipes[0], include_fields=API_RECIPE_INCLUDE_FIELDS) if recipes else {}
        cache.set(cache_name, rendered, CACHE_TIMEOUT_RECIPES_BEST_TODAY)

    return rendered


def get_or_set_recipe_best_month_cached() -> Dict[str, List[Dict[str, str]]]:
    cache_name = f'{__name__}_best_month'

    rendered = cache.get(cache_name)
    if not rendered:
        recipes = get_best_recipes_by_start_date(datetime.now() - timedelta(days=30), count=2)
        rendered = render_recipes(recipes, include_fields=API_RECIPE_INCLUDE_FIELDS)
        cache.set(cache_name, rendered, CACHE_TIMEOUT_RECIPES_BEST_MONTH)

    return rendered


def get_or_set_recipe_best_feasts_cached() -> Dict[str, List[Dict[str, str]]]:
...
def get_or_set_recipe_groups_cached() -> Dict[str, List[Dict[str, str]]]:
    ...
    if not rendered:
        ...
        for i in range(len(best_groups)):
            ...
            rendered['groups'][i]['recipes'] = render_recipes(recipes, include_fields=API_RECIPE_INCLUDE_FIELDS)
